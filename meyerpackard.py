import numpy import random import operator from bs4 import BeautifulSoup as bs import urllib.request as urllib2 import csv

random.seed(a=None)

Set the Constants
---------------PopulationSize = 200 DataSize = 0 NumberOfGenerations = 4 MutationRate = 5 MutationChange = 2 Stock_name = 'tsla' NumReturn = 5 NumberOfDay = 365 File_path = '2Improvement.csv' #----------------------------------

class Chromosome(): def init(self, min=None, max=None, prev_min=None, prev_max=None, buy=None, score =None): self.min = min self.max = max self.prev_min = prev_min self.prev_max = prev_max self.buy = buy self.score = score
...
class TrainingData(object): population = [] nextGeneration = [] dayChange = [] nextDayChange = [] profit = []
...
def csv_dict_writer(path, fieldnames, data):
    ...
    for row in data:
        writer.writerow(row)
    out_file.close()
if name == 'main': x = TrainingData() x.generateData() x.populationInit() x.weighted_random_choice() x.uniformCross(MutationRate) x.printChromosomes()
